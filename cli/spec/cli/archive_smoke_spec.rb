...
RSpec.describe "Archive smoke" do
  ...
  before do
    ...
    allow(@mb).to receive(:preflight).with(existing_uids: []).and_return({uidvalidity: 77, to_fetch: [11, 12], server_size: 2})
  ...
  Then "archives files and is idempotent" do
    ...
    expect { cli.invoke(:archive, [], mailbox: mailbox, address: address, output: archive_base) }.not_to raise_error
